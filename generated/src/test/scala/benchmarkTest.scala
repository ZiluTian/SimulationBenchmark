...
object benchmarkTest {
    def main(args: Array[String]): Unit = {
        ...
        val agents = example match {
            case "wator" => generated.ticktalk.example.cellularAutomata.wator.InitData(width, margs(0).toInt)
            case "epidemic" => generated.ticktalk.example.epidemic.InitData(margs(0).toInt)  // first argument
            case "gameOfLife" => generated.ticktalk.example.gameOfLife.InitData(width, margs(0).toInt)
            case "segregation" => generated.ticktalk.example.segregation.InitData(width, margs(0).toInt, margs(1).toInt)
        }
